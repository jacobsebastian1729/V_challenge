...
def apply_threshold(probs, threshold=0.5):
    ...
    return np.where(probs <= threshold, 'Low', 'Not Low')

def apply_threshold_hm(probs, threshold=0.5):
    print("threshold", threshold)
    return np.where(probs <= threshold, 'High', 'Medium')
...
y_train_low_vs_rest.head()
print(y_train_low_vs_rest.value_counts())

scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)

# Initialize and train the classifiers
classifier_low_vs_rest = SVC(kernel='poly', probability=True, random_state=42)
...
y_pred_4_low_prob = classifier_low_vs_rest.predict_proba(X_test4_scaled)[:, 1]

threshold = 0.15
final_predictions_4 = apply_threshold(y_pred_4_low_prob, threshold)
...
high_medium_filter = y_train != 'Low'
X_train_high_medium = X_train[high_medium_filter]
y_train_high_vs_medium = y_train[high_medium_filter]
X_train_high_medium_scaled = scaler.transform(X_train_high_medium)

#classifier_high_vs_medium = SVC(kernel='poly', probability=True, random_state=42)
classifier_high_vs_medium = SVC(kernel='linear', decision_function_shape='ovr', probability=True, random_state=42)
classifier_high_vs_medium.fit(X_train_high_medium_scaled, y_train_high_vs_medium)

X_test4_hm = df2_test4.drop(columns=['ChassisId_encoded', 'gen', 'risk_level', 'v_category'])
X_test4_hm = X_test4_hm[high_medium_filter]
X_test4_hm_scaled = scaler.transform(X_test4_hm)

# Predict probabilities for each class
y_pred_4_hm_prob = classifier_high_vs_medium.predict_proba(X_test4_hm_scaled)[:, 1]

threshold = 0.4
final_predictions_4_hm = apply_threshold_hm(y_pred_4_hm_prob, threshold)
final_predictions_4_hm = pd.Series(final_predictions_4_hm, index=X_test4_hm.index)
...
# Create a DataFrame for true labels and predictions
y4_true_hm = pd.DataFrame()
y4_true_hm['risk_level'] = y_train_high_vs_medium
y4_true_hm['pred'] = final_predictions_4_hm

print(y4_true_hm['risk_level'].value_counts())
print(y4_true_hm['pred'].value_counts())
score4_hm = macro_avg_f1(y4_true_hm['risk_level'], y4_true_hm['pred'])
print('\nScores4_hm Details:\n', score4_hm)
accuracy = (y4_true_hm['risk_level'] == y4_true_hm['pred']).mean()
print("Accuracy:", accuracy)
...
print(y_pred_1_low_prob)

threshold = 0.15
final_predictions_1 = apply_threshold(y_pred_1_low_prob, threshold)
final_predictions_1 = pd.Series(final_predictions_1, index=X_test1.index)

# Evaluate the results
df2_test1_ = df2_test1.copy()
df2_test1_['risk_level_binary'] = df2_test1['risk_level'].apply(lambda x: 'Low' if x == 'Low' else 'Not Low')
...
not_low_indices = y1_true[y1_true['pred'] == 'Not Low'].index
print("not_low_indices_1",len(not_low_indices))
X_test1_high_medium = X_test1.iloc[not_low_indices]
#y_test1_high_vs_medium = y_train[high_medium_filter]
X_test1_high_medium_scaled = scaler.transform(X_test1_high_medium)

y_pred_1_hm = classifier_high_vs_medium.predict_proba(X_test1_high_medium_scaled)[:, 1]
print(y_pred_1_hm)
print(len(y_pred_1_hm))

threshold = 0.4
final_predictions_1_hm = apply_threshold_hm(y_pred_1_hm, threshold)
final_predictions_1_hm = pd.Series(final_predictions_1_hm, index=X_test1_high_medium.index)
matching_indices_1 = y1_true.index.intersection(final_predictions_1_hm.index)

# Update the 'pred' column in y1_true where indices match
y1_true.loc[matching_indices_1, 'pred'] = final_predictions_1_hm.loc[matching_indices_1]
...
print(y_pred_low_prob)

threshold = 0.15
final_predictions = apply_threshold(y_pred_low_prob, threshold)
...
not_low_indices = y_true[y_true['pred'] == 'Not Low'].index
print("not_low_indices_1",len(not_low_indices))
X_test_high_medium = X_test.iloc[not_low_indices]
#y_test1_high_vs_medium = y_train[high_medium_filter]
X_test_high_medium_scaled = scaler.transform(X_test_high_medium)

y_pred_hm = classifier_high_vs_medium.predict_proba(X_test_high_medium_scaled)[:, 1]
#print(y_pred_1_hm)
print(len(y_pred_hm))

threshold = 0.4
final_predictions_hm = apply_threshold_hm(y_pred_hm, threshold)
final_predictions_hm = pd.Series(final_predictions_hm, index=X_test_high_medium.index)
matching_indices = y_true.index.intersection(final_predictions_hm.index)

# Update the 'pred' column in y1_true where indices match
y_true.loc[matching_indices, 'pred'] = final_predictions_hm.loc[matching_indices]
print(y_true['pred'].value_counts())
